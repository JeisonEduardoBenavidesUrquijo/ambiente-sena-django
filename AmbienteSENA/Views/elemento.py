    ...
    if request.method == 'POST':
        if request.POST.get('nombre') and request.POST.get('tipo') and request.POST.get('observacion') and request.POST.get('ambiente') and request.FILES.get('foto'):
            try:
                ...
                messages.error(request, f'Ocurrio un error en el sistema. Intentelo mas tarde { e }')
...
def ListarElementos(request):
    try:
        ...
        {'listadoelementos':listadoelementos,'ambientes':listadoambientes})
    except DatabaseError as e:
...
def ActualizarElemento(request):
    try:
        ...
        if request.method == 'POST':
            if(request.POST.get('nombre') and request.POST.get('tipo') and request.POST.get
            ('observacion') and request.POST.get('ambiente')):
                elemento = Elemento()
    ...
    if request.method == 'POST':
        if request.POST.get('id'):
            try:
                ...
                print("XXXXXXXXXX ", ruta_imagen)
                if ruta_imagen.exists():
                    ##print("La ruta de la imagen a eliminar es: ", ruta_imagen)
